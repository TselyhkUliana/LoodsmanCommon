...
namespace LoodsmanCommon
{
  ...
  public static class NetPluginCallExtensions
  {
    ...
    /// <br/>[_IDACTOR] int - идентификатор пользователя(дубликат);
    /// <br/>[_NAME] string - имя пользователя;
    /// <br/>[_FULLNAME] string - полное имя пользователя;
    /// <br/>[_MAIL] string - адрес электронной почты пользователя;
    /// <br/>[_WORKDIR] string - рабочая папка пользователя;
    /// <br/>[_FILEDIR] string - папка для хранения файлов пользователя;
    /// <br/>[_PROFILE_ID] int - идентификатор профиля пользователя;
    /// <br/>[_PROFILE] string - профиль пользователя;
    /// <br/>[_WINUSER] short - признак пользователя Windows (1 - является пользователем Windows, 0 - не является пользователем Windows); 
    /// <br/>[_PHONE] string - номер телефона пользователя;
    /// <br/>[_SKYPE] string - адрес пользователя в Skype;
    /// <br/>[_IM] string - № IM;
    /// <br/>[_WEBPAGE] string - адрес Web-страницы пользователя;
    /// <br/>[_NOTE] string - примечание;
    /// <br/>[_STATUS] short - статус пользователя (0 - доступен, 1 - недоступен, 2 - уволен);
    /// <br/>[_MAINPOSITION_ID] int - идентификатор основной должности пользователя;
    /// <br/>[_PATH] string - путь (возвращает пустое значение);
    /// <br/>[_IS_ADMIN] int - Возвращает признак того, является ли пользователь, администратором;
    /// <br/>[_IS_MISSING_USER] int - всегда возвращает 0;
    /// </summary>
    public static DataTable Native_GetUserList(this INetPluginCall pc) =>
    ...
    /// <summary> Возвращает информацию об атрибуте. </summary>
    /// <param name="name"> Название атрибута. </param>
    /// <param name="mode"> Режим вывода. В зависимости от его значения выдается соответствующая информация. </param>
    /// <returns> Зависит от режима <see cref="GetInfoAboutAttributeMode"/>. </returns>
    public static DataTable Native_GetInfoAboutAttribute(this INetPluginCall pc, string name = null, GetInfoAboutAttributeMode mode = GetInfoAboutAttributeMode.Mode2) =>
        pc.GetDataTable("GetInfoAboutAttribute", name, mode);
